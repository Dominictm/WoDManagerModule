...
**Локации в домене** (события в основном городе/регионе хроники):
```
locations/[Район]/[Название_локации]/[Название].md
```

**Локации вне домена** (другие города, страны, внедоменные события модуля):
```
locations/Другие/[Название_модуля]/[Название_локации]/[Название].md
```

> Пример: локация «Замок в горах» для модуля `январь_2024_замок` →
> `locations/Другие/январь_2024_замок/Замок_в_горах/Замок_в_горах.md`
...
- **Ночь** — никакого дневного света; адаптируйте под свой домен и год
- **Мокрые поверхности** — дождь, туман, отражения
- **Контраст тёплого и холодного** — янтарные фонари vs холодное синее небо
- **Архитектура домена** — фасады, мостовые, специфические материалы вашего города
...
Визуальные референсы: ночные фотографии вашего города + концепт-арт VtM: Bloodhunt + нуар-кинематограф
...
**Блок 1 — Место:**
`[конкретное описание локации], [ГОРОД] [район] [год], night`
...
|---|---|---|
| 1 | Место и время | `[описание локации], [ГОРОД] [год], night` |
...
- `neon signs` — [ночной квартал домена], ночные клубы
...
- ❌ Современные элементы позже установленного года хроники
